...
const getUser = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        status: "error",
        message: "error getting a user. Please try again",
      });
  }
};
...
const addUser = async (req, res) => {
  try {
    ...
    let { error, value } = validatedSchema.validate({
      email,
      firstName,
      lastName,
      birthDate,
    ...
    if (checkIfUserExists) {
      return res
        .status(409)
        .json({
          message: "user already exists. Try a different email address",
        });
    }
...
const updateUser = async (req, res) => {
  try {
    ...
    return res
      .status(404)
      .json({ status: "client error", message: "user does not exist" });
  } catch (err) {
    console.log("error updating user:", err.message, err.code);
    return res
      .status(500)
      .json({ status: "server error", message: "error updating user." });
  }
};
...
const loginUser = async (req, res) => {
  try {
    ...
    if (!checkIfUserExists) {
      return res
        .status(404)
        .json({ status: "not found", message: "user was not found" });
    }

    // compare password

    let comparedPwd = await bcrypt.compare(
      password,
    ...
    if (!comparedPwd) {
      return res
        .status(401)
        .json({ status: "error", message: "password is invalid" });
    }

    let token = createToken(
      checkIfUserExists.id,
      checkIfUserExists.email,
      checkIfUserExists.birthDate,
      checkIfUserExists.firstName,
      checkIfUserExists.lastName
    );
    res.cookie("clurk_access_token", token, {
      maxAge: 60 * 60 * 60 * 24 * 24 * 24 * 7,
      secure: true,
    ...
    return res
      .status(500)
      .json({ status: "error", message: "error logging in user" });
  }
};
